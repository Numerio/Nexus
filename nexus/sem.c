	...
	bigtime_t timeout)
{
	...
	if (flags & B_ABSOLUTE_TIMEOUT) {
		has_deadline = true;
		deadline_ns = (uint64_t)timeout * 1000ULL;
		...
		has_deadline = true;
		deadline_ns = ktime_get_ns() + (uint64_t)timeout * 1000ULL;
	}
	...
	for (;;) {
		...
		if (has_deadline) {
			ktime_t deadline = ns_to_ktime((int64_t)deadline_ns);
...
void nexus_sem_team_exit(pid_t team)
{
	...
	idr_for_each_entry(&nexus_sem_idr, sem, id) {
		...
		list_add(&sem->waiters, &dead);
	}
	spin_unlock_irqrestore(&sem_idr_lock, flags);

	while (!list_empty(&dead)) {
		sem = list_first_entry(&dead, struct nexus_sem, waiters);
		list_del_init(&sem->waiters);
		nexus_sem_put(sem);
	}
}
...
static long nexus_sem_ioctl_create(unsigned long arg)
{
	...
	ex.ret = nexus_create_sem(ex.count, ex.name, &id);
	if (ex.ret == B_OK)
		ex.id = id;
...
static long nexus_sem_ioctl_acquire(unsigned long arg)
{
	...
	ex.ret = nexus_acquire_sem(ex.id, ex.count, ex.flags, ex.timeout);
...
static long nexus_sem_ioctl_release(unsigned long arg)
{
	...
	ex.ret = nexus_release_sem(ex.id, ex.count, ex.flags);
...
static long nexus_sem_ioctl_count(unsigned long arg)
{
	...
	ex.ret = nexus_get_sem_count(ex.id, &ex.count);
...
static long nexus_sem_ioctl_info(unsigned long arg)
{
	...
	ex.ret = nexus_get_sem_info(ex.id, &ex.info);
...
static long nexus_sem_ioctl_next_info(unsigned long arg)
{
	...
	ex.ret = nexus_get_next_sem_info(ex.team, ex.cookie, &ex);
